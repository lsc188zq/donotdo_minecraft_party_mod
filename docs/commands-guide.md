...
| 命令 | 权限 | 作用 |
| --- | --- | --- |
| `/party setarena [x y z]` | 管理员 | 站在场地中心点执行：落地当前选中地图并设为竞技场中心；可选 x y z 指定安全点坐标 |
...
**安全点（建场前的传送）**：落地前会把竞技场区域内的所有玩家传送到安全点，防止区域清空时坠坑或被覆盖。可选坐标参数 `x y z` 指定安全点位置（如 `/party setarena 100 200 100`）；不指定时默认传送到**竞技场中心正上方 120 格**。传送附带缓降效果，高空落地不会摔伤。移出前会保存玩家快照，游戏结束/`/party stop` 后优先恢复到 setarena 前的位置（"家"）。

**注意**：`setarena` 会**清空竞技场区域**（±地图半径的立方体）内的所有方块与建筑——场地请选在空旷处，区域内的重要建筑会被清除。区域外完全不受影响。
...
列出"程序生成房"、内置地图（标注 `[内置]`，随 mod 打包分发）、自建地图，当前选中的标注 `[当前]`。内置地图不可删除。
